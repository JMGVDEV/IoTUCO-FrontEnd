...
class ConfigActions extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            greenHouse: [], growBed: [], startHour: '', finalHour: ''
        }
    }
    ...
    handleChange = e => {
        this.setState({
            [e.target.greenHouse]: e.target.value,[e.target.growBed]: e.target.value
        })
    }
    ...
    render() {
        ...
        return (
            ...
            <div className="container"  >
                ...
                <Grid container justify="center"  >
                    <DropdownButton id="dropdown-basic-button" title="CAMAS" onChange={this.selectOption}>
                        {this.state.growBed && (this.state.growBed.map(growbed =>
                             <DropdownItem onClick={(e) => { this.getIdBed(e, growbed) }} key={growbed.grow_bed}>{growbed.grow_bed}</DropdownItem>))}
                    </DropdownButton>
                </Grid>
                ...
                <div style={{ padding: 30 }}>
                    <Grid container justify="center"  >
                        <Button onClick={this.hourConfig} variant="contained" color="primary" className='configure'>
                            CONFIGURAR
                        </Button>
                    </Grid>
                </div>
                ...
                <Grid container justify="center" >
                    <div className="btn-group" >
                        <DropdownButton id="dropdown-basic-button" title="INVERNADEROS" onChange={this.selectOption} >
                            {this.state.greenHouse && (this.state.greenHouse.map(greenhouse => <DropdownItem onClick={(e) => { this.getIdHouse(e, greenhouse) }} key={greenhouse.green_house}>{greenhouse.green_house}</DropdownItem>))}
                        </DropdownButton>
                        <Button></Button>
                        <Button onClick={this.openBlinds} variant="contained" color="primary" className='config'>
                            ABRIR CORTINAS
                        </Button>{' '}
                    </div>
                </Grid>
